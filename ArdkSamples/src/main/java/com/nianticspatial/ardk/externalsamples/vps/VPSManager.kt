...
import com.nianticlabs.ardk.VPSConfig
import com.nianticlabs.ardk.vps.VPSSession
...
class VPSManager(
   ...
   */
  fun createAnchor() {
    val trackedCount = localAnchorPayloadCache.size + if (_targetAnchorId.value != null) 1 else 0
    if (trackedCount >= MAX_TRACKED_ANCHORS) return

    ardkManager.currentFrame?.let { frame ->
      runCatching {
  ...
  fun startTracking(payload: String) {
    ...
    } catch (error: ArdkStatusException) {
  ...
  private fun updateGeolocation() {
    ardkManager.currentFrame?.camera?.let { camera ->
      when (val result = session.getDevicePoseAsGeolocation(camera.pose)) {
        is ARDKResult.Success -> {
          _geolocationInfo.value = "Lat: %.3f, Lon: %.3f, Heading: %.3f, Alt: %.3f"
            .format(result.value.latitude, result.value.longitude,
