...
fun SemanticsView(
    ...
    val semanticsManager = remember { SemanticsManager(ardkSessionManager) }
    val overlayContent = remember { SemanticsOverlayContent(semanticsManager, ardkSessionManager) }
    ...
    DisposableEffect(lifecycleOwner, semanticsManager) {
        lifecycleOwner.lifecycle.addObserver(semanticsManager)
        onDispose {
            lifecycleOwner.lifecycle.removeObserver(semanticsManager)
            semanticsManager.stop()
        }
    }

    LaunchedEffect(semanticsManager) {
        semanticsManager.start()
    }

    var latestWarning by remember { mutableStateOf<SemanticsWarningEvent?>(null) }
    LaunchedEffect(semanticsManager) {
        semanticsManager.warnings.collect { event ->
            latestWarning =
                if (event == SemanticsWarningEvent.Cleared) null else event
        }
    }

    val channels by semanticsManager.channels.collectAsState()
    val selectedIndex by semanticsManager.selectedChannelIndex.collectAsState()
    val sessionState by semanticsManager.sessionState.collectAsState()
    val overlayOpacity by semanticsManager.overlayOpacity.collectAsState()
        ...
        Column(
            ...
            Column(
                ...
                ExposedDropdownMenuBox(
                    ...
                    val selectedChannel = channels.getOrNull(selectedIndex)?.let(::formatChannelName)
                    OutlinedTextField(
                    ...
                    ExposedDropdownMenu(
                        ...
                        channels.forEachIndexed { index, channel ->
                            val prettyName = formatChannelName(channel)
                            DropdownMenuItem(
                                text = { Text(prettyName) },
                                onClick = {
                                    semanticsManager.selectChannel(index)
                                    overlayContent.reset()
            ...
            Column(
                ...
                Slider(
                    ...
                    onValueChange = { value ->
                        semanticsManager.setOverlayOpacity(value)
...
private fun formatChannelName(raw: String): String =
    raw
        .replace('_', ' ')
        .split(' ')
...
private fun StatusCard(
    ...
    val statusText = when (sessionState) {
        ...
        SemanticsSessionState.LoadingChannels -> "Loading semantic segmentation labels..."
        is SemanticsSessionState.Streaming -> {
            val pretty = sessionState.channelName?.let(::formatChannelName)
            pretty?.let { "Streaming $it" } ?: "Streaming semantics"
        }
    ...
    val warningText = when (warningEvent) {
        ...
        is SemanticsWarningEvent.LatestConfidenceQueryFailed ->
            "Latest confidence failed (channel ${warningEvent.channelIndex}): " +
                warningEvent.cause.userFacingMessage()
