  ...
  companion object {
    ...
    fun createMeshNodes(parentNode: PoseNode) {
      ...
      renderableMeshChunks.forEachIndexed { index, chunk ->
        ...
        if (hasUvs) {
          correctedUvs = FloatArray(uvs.size) { i -> uvs[i]
          }
        } else {
        ...
        val vertexBufferBuilder = VertexBuffer.Builder()
          ...
          .attribute(VertexBuffer.VertexAttribute.POSITION, 0, VertexBuffer.AttributeType.FLOAT3, 0, vertexStride)

        if (hasUvs) {
          vertexBufferBuilder.attribute(VertexBuffer.VertexAttribute.UV0, 1, VertexBuffer.AttributeType.FLOAT2, 0, uvStride)
        }
        ...
        if (chunk.textureData == null) {
          ...
          val bitmap = BitmapFactory.decodeByteArray(chunk.textureData, 0, chunk.textureData.size)
          if (bitmap != null) {
            ...
            texture.setImage(engine, 0, Texture.PixelBufferDescriptor(byteBuffer, Texture.Format.RGBA, Texture.Type.UBYTE))
        ...
        meshNode.transform = Mat4( x = Float4( chunk.modelMatrix[0], chunk.modelMatrix[1], chunk.modelMatrix[2], chunk.modelMatrix[3]),
                                y = Float4( chunk.modelMatrix[4], chunk.modelMatrix[5], chunk.modelMatrix[6], chunk.modelMatrix[7]),
                                z = Float4( chunk.modelMatrix[8], chunk.modelMatrix[9], chunk.modelMatrix[10], chunk.modelMatrix[11]),
                                w = Float4( chunk.modelMatrix[12], chunk.modelMatrix[13], chunk.modelMatrix[14], chunk.modelMatrix[15]))
