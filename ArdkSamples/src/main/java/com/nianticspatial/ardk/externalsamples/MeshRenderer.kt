...
class MeshRenderer(
    ...
    fun setDefaultMaterialColor(color: Color) {
        _defaultMaterialColor = color
    }

    // Clear the existing mesh chunks and add the new ones (if any)
    fun setRenderableMeshChunks(meshChunks: List<RenderableMeshChunk>) {
        renderableMeshChunks.clear()
        renderableMeshChunks.addAll(meshChunks)
    ...
    fun createMeshNodes(parentNode: PoseNode) {
        ...
        renderableMeshChunks.forEachIndexed { index, chunk ->
            ...
            if (chunk.textureData == null) {
                Log.d(TAG, "Chunk $index: No texture data. Using dummy.")
