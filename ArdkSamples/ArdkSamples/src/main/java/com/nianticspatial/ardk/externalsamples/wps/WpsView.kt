package com.nianticspatial.ardk.externalsamples.wps
...
import com.nianticspatial.ardk.externalsamples.arChildNodes
import com.nianticspatial.ardk.externalsamples.common.ErrorDisplay
import com.nianticspatial.ardk.externalsamples.createUnlitColorMaterial
import com.nianticspatial.ardk.externalsamples.destroyRecursively
...
fun WpsView(
  ...
  val wpsManager = remember(context, ardkSessionManager) {
    WpsManager(ardkSessionManager)
  }
  val gpsManager = remember(context, ardkSessionManager) {
    GpsManager(ardkSessionManager)
  }
  ...
  LaunchedEffect(cubePoseNode, tappedPosition, trackingToEdn, wpsOriginLatitude, wpsOriginLongitude, wpsLongitude, wpsLatitude, wpsHeadingDeg) {
    ...
    val cameraPose = ardkSessionManager.currentFrame?.camera?.pose
    cameraPose?.let {
      world.y = it.ty()
      node.position = world
